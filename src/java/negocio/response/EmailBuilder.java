...
/**
 * Clase Email Builder. Se encarga de recibir la lista de persona y enviar email
 * a cada una Recorre la lista enviada y retorna el resultado de la operación
 *
 * @author javie
 */
public class EmailBuilder {
    ...
    public boolean enviarEmails(IExcelContent tabla, String contenidoMensaje, String asunto, int columna) throws Exception{
        ...
        for (int i = 0; i < tabla.getDatos().size(); i++) {
            ...
            LogDTO dto = new LogDTO("Envío de correo", "Enviando correo a: "+tabla.getDatos().get(i).get(columna), entrada.toString(), entrada.toString());
    ...
    private String buildBody(int posicion, String contenido, IExcelContent tabla) throws Exception {
        String bodyEmail = contenido;
        java.util.Date date = new java.util.Date();
            Date entrada = new Date(date.getTime());
            LogDAO dao = new LogDAO();
            LogDTO dto = new LogDTO("Construcción de email", "Construyendo cuerpo del mensaje con los datos reales ", entrada.toString(), entrada.toString());
            dao.registrarLog(dto);
